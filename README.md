...
5. **Equipamentos nomeados** — aeroportos, campi, estádios, shoppings, hospitais e parques aparecem como pontos próprios (`AIR_*`, `UNI_*`, … na taxonomia do depot). Eles **capturam** a demanda que a pesquisa já manda para a zona deles, em vez de criar demanda nova por um modelo de gravidade: as origens continuam sendo as da matriz O-D e a população total não muda. A captura é uma **fatia proporcional de cada pop** que chega à zona, limitada à capacidade de um dia típico e a `poi_max_zone_share` da demanda disponível — assim o equipamento não estoura a própria capacidade, não esvazia a zona e herda as origens na mesma proporção em que elas existem.
