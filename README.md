...
6. **Equipamentos nomeados** — escolas, campi, hospitais, shoppings, parques, estádios, aeroportos e rodoviárias vêm do **OpenStreetMap** (cada ponto guarda o `osm_id`; são ~3,7 mil com demanda própria), e o **porte** de cada um é medido pela atividade não-residencial **dentro do contorno que o OSM registra para ele** (com uma folga de `poi_margin_m`); onde o OSM não traz geometria, o porte fica limitado a `poi_spread_above`, porque medir o entorno de uma escola de bairro lhe daria os prédios do quarteirão — a mesma medida que posiciona os pontos de demanda. Não há capacidade escrita à mão; medir num raio fixo faria uma praça de esquina herdar os prédios do quarteirão inteiro. Eles **capturam** parte da demanda que a pesquisa já manda para a zona, em fatias proporcionais de cada pop que chega e só entre os motivos que o tipo atende, limitados a `poi_max_zone_share` da zona.
...
| Equipamentos (`pois.csv`) | escolas, campi, hospitais, shoppings, parques, estádios, aeroportos | OpenStreetMap, via Overpass |
