    ...
    with zipfile.ZipFile(settings.cnefe_zip) as z:
        ...
        with z.open(name) as raw, open(out, "w", encoding="ascii") as fout:
            ...
            for line in raw:
                ...
                kept += 1
    log.info("CNEFE: lidos=%d mantidos=%d -> %s (%.1f MB)", n, kept, out.name, _mb(out))
    ...
    with zipfile.ZipFile(settings.censo_zip) as z:
        ...
        with z.open(name) as raw, open(out, "w", encoding="ascii") as fout:
            ...
            for line in raw:
                ...
                kept += 1
    log.info("Censo: lidos=%d setores-SP=%d -> %s", n, kept, out.name)
